package com.afx.web.receiptorganizer.rest.label;
...
import com.afx.web.receiptorganizer.rest.model.request.label.EditLabelRequest;
import com.afx.web.receiptorganizer.rest.model.response.BaseResponse;
import com.afx.web.receiptorganizer.rest.model.response.label.LabelsResponse;
...
public class LabelController {
    ...
    public LabelsResponse getLabels(@ModelAttribute("user") User user, Locale locale) {
        ...
        try {
            response.setLabels(this.LabelService.getAllLabels(user.getUsername()));
    ...
    public BaseResponse createLabel(@ModelAttribute("user") User user, @Valid @RequestBody Label label,
        ...
        if (!result.hasErrors()) {
            try {
                this.LabelService.addLabel(user.getUsername(), label);
                response.setSuccess(true);
    ...
    public BaseResponse deleteLabel(@ModelAttribute("user") User user, @Valid @RequestBody Label label,
        ...
        if (!result.hasErrors()) {
            try {
                this.LabelService.deleteLabel(user.getUsername(), label);
                response.setSuccess(true);
    ...
    public BaseResponse editLabel(@ModelAttribute("user") User user, @Valid @RequestBody EditLabelRequest request, BindingResult result, Locale locale) {
        ...
        if (!newLabel.getName().equals("") && !newLabel.getName().equals("All Receipts")
                && this.LabelService.isLabelUnique(user.getUsername(), newLabel)) {

            logger.debug("User submitted valid request to create new label.");
            this.LabelService.editLabel(user.getUsername(), oldLabel, newLabel);
            response.setSuccess(true);
